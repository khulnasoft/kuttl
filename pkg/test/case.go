...
func (t *Case) CreateNamespace(namespace string, annotations, labels map[string]string) error {
...
func (t *Case) LoadTestSteps(annotations, labels map[string]string) error {
	...
	if ns == "" {
		...
		if err := t.CreateNamespace(ns, annotations, labels); err != nil {
			return err
		}
		t.Namespace = ns
	}
